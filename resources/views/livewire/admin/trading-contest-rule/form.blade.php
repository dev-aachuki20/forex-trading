<form wire:submit.prevent="{{ $updateMode ? 'update' : 'store' }}" class="tablelist-form" autocomplete="off">
    ...
    <div class="row mb-3">
        ...
        <div class="row mb-2">
            ...
            <div class="col-2">
                @if($index != 0)
                <button class="btn btn-danger" wire:click.prevent="removeRow({{ $index }})"><i class="ri-subtract-line"></i></button>
                @endif
                @if($index == count($ruleContent)-1)
                <button class="btn btn-primary" wire:click.prevent="addRow({{ $index+1 }})"><i class="ri-add-line"></i></button>
                @endif
            </div>
        </div>
        <div class="row mb-2">
            <div class="col-10" wire:ignore>
                <textarea id="summernote" class="form-control" wire:model="ruleContent.{{ $index }}.description" rows="3" placeholder="{{$allKeysProvider['description']}}"></textarea>
            </div>
