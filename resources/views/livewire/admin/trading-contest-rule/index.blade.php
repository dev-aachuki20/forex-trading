            ...
            <div class="row">
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        <h4 class="mb-sm-0">{{$allKeysProvider['trading_contest_rules']}}</h4>
                    </div>
                </div>
            </div>
            <!-- end page title -->
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            @if ($formMode)
                            @include('livewire.admin.trading-contest-rule.form', ['languageId' => $languageId])
                            @elseif($viewMode)
                            @livewire('admin.trading-contest-rule.show', ['rule_id' => $rule_id])
                            @else
                            <div class="listjs-table" id="customerList">
                                ...
                                <div class="table-responsive mt-3 my-team-details table-record">
                                    <table class="table table-striped table-hover">
                                        ...
                                        <tbody>
                                            @if($allContestRule->count() > 0)
                                            @foreach($allContestRule as $contestrule)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ ucfirst($contestrule->title)}}</td>
                                                <td>
                                                    <label class="switch">
                                                        <input wire:click.prevent="toggle({{ $contestrule->id }},{{$loop->iteration}})" id="switch-input-{{$activeTab}}-{{$loop->iteration}}" class="switch-input" type="checkbox" {{ $contestrule->status == 1 ? 'checked' : '' }} />
                                                        <span class="switch-label" data-on="{{ $statusText }}" data-off="deactive"></span>
                                                        <span class="switch-handle"></span>
                                                    </label>
                                                </td>
                                                <td>{{ convertDateTimeFormat($contestrule->created_at,'date') }}</td>

                                                <td>
                                                    <div class="d-flex gap-2">
                                                        <div class="view">
                                                            <button type="button" wire:click="show({{$contestrule->id}})" class="btn btn-sm btn-primary view-item-btn"><i class="ri-eye-fill"></i></button>
                                                        </div>

                                                        <div class="edit">
                                                            <button type="button" wire:click="edit({{$contestrule->id}})" class="btn btn-sm btn-success edit-item-btn"><i class="ri-edit-box-line"></i></button>
                                                        </div>

                                                        <div class="remove">
                                                            <button type="button" wire:click.prevent="delete({{$contestrule->id}})" class="btn btn-sm btn-danger remove-item-btn"><i class="ri-delete-bin-line"></i></button>
                                                        </div>

                                                        <div class="remove">

                                                            <a type="button" href="{{ route('auth.contestrule-rules', $contestrule->id) }}" class="btn btn-sm btn-primary remove-item-btn"><i class="ri-add-line"></i></a>
                                                        </div>
                                                    </div>
                                                </td>
                                            </tr>
                                ...
                                {{ $allContestRule->links('vendor.pagination.bootstrap-4') }}
                                <!-- eng tab end -->
                            </div>
                            @endif
                        </div>
