<section class="left-text-sec overlay-dark-gradient1 padding-tb-100" style="background-image:url({{ $sectionDetail->image_url ? $sectionDetail->image_url : asset('images/ads-bg2.jpg') }})">
    <div class="container position-relative z-10">
        <div class="row justify-content-center">
            <div class="col-lg-6 col-md-8 col-sm-12">
                <div class="section-head text-white text-center">
                    <h2 class="text-white">{{ $sectionDetail ? ucwords($sectionDetail->title) : 'Lorem Ipsum is simply dummy' }}</h2>
                    <div class="discription">
                        <p>{!! $sectionDetail ? ucfirst($sectionDetail->description) : "Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy" !!} </p>
                    </div>
                    <div class="button-group justify-content-center">
                        <a class="custom-btn outline-color-white" href="#">{{ $allKeysProvider['start_trading'] ?? 'Start Trading'}}</a>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>